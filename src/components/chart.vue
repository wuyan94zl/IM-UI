          ...
          <mt-tab-container-item id="聊天">
            ...
            >
              ...
              <li v-for="item in list" :key="item">
                {{ item.nick_name }}
                <mt-button @click="sendMessage(item.channel_id)"
                  >发送消息</mt-button
                >
              </li>
            </ul>
          </mt-tab-container-item>
          <mt-tab-container-item id="好友"> </mt-tab-container-item>
      ...
      <mt-popup v-model="qiyeVisible" position="right">
        ...
        <div style="margin: 40px 5px 5px 5px; width: 100%">
          ...
          >
            ...
            ></mt-field>
            <mt-button class="is-right" style="float: right; width: 100px"
              >发送</mt-button
            >
          </div>
        </div>
  ...
  methods: {
    ...
    sendMessage() {
      let obj =
        '{"channel_id": "' +
        this.channelId +
        '","content": "发送的消息","type": "3"}';
      this.$SOCKET.sendMessage(obj);
    },
    loadMore() {
      this.loading = true;
      this.$axios.post("http://localhost:8888/friend/list", {}).then((res) => {
